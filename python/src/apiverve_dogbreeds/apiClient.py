...
class DogbreedsAPIClientError(Exception):
...
class ValidationError(DogbreedsAPIClientError):
...
class DogbreedsAPIClient:
    ...
    def __init__(self, api_key: str, secure: bool = True, debug: bool = False):
        """
        Initialize the DogbreedsAPIClient with the API key and optional settings

        :param api_key: Your APIVerve API key
        :param secure: Deprecated. Always set to True
        :param debug: Enable debug logging (default: False)
        :raises DogbreedsAPIClientError: If API key is invalid
        """
    ...
    def _log(self, message: str) -> None:
        ...
        """
        if self.debug:
            print(f"[DogbreedsAPIClient DEBUG] {message}")
    ...
    def execute(self, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:  # Returns DogBreedsData in 'data' field
        """
        Execute the API request

        :param params: API parameters (optional)
        :return: API response as a dictionary
        :raises DogbreedsAPIClientError: If the request fails or returns an error
        ...
        try:
            ...
            raise DogbreedsAPIClientError(f"Request timeout: {str(e)}")
        except requests.exceptions.ConnectionError as e:
            self._log(f"Connection error: {str(e)}")
            raise DogbreedsAPIClientError(f"Connection error: {str(e)}")
        except requests.exceptions.RequestException as e:
            self._log(f"Request exception: {str(e)}")
            raise DogbreedsAPIClientError(f"Request failed: {str(e)}")

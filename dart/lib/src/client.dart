...
class DogbreedsValidationException implements Exception {
  final List<String> errors;

  DogbreedsValidationException(this.errors);

  @override
  String toString() => 'DogbreedsValidationException: ${errors.join("; ")}';
}
...
/// Dog Breeds API client.
///
/// For more information, visit: https://apiverve.com/marketplace/dogbreeds?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [breed] (required) - The name of the dog breed to get information about
class DogbreedsClient {
  final String apiKey;
  ...
  static final Map<String, ValidationRule> _validationRules = <String, ValidationRule>{
    'breed': ValidationRule(type: 'string', required: true),
  };

  DogbreedsClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/dogbreeds',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [DogbreedsValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw DogbreedsValidationException(errors);
    }
  }

  /// Execute a request to the Dog Breeds API.
  ///
  /// Parameters are validated before sending the request.
  Future<DogbreedsResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw DogbreedsException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return DogbreedsResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw DogbreedsException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw DogbreedsException('Resource not found');
      } else {
        throw DogbreedsException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is DogbreedsException) rethrow;
      throw DogbreedsException('Request failed: $e');
    }
  }
...
/// Exception thrown by the Dog Breeds API client.
class DogbreedsException implements Exception {
  final String message;

  DogbreedsException(this.message);

  @override
  String toString() => 'DogbreedsException: $message';
}
